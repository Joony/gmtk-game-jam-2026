...
func _run() -> void:
	...
	# The game auto-starts on load now, so Esc is live immediately — there is no START prompt
	# to gate it behind any more.
	_check("pause menu enabled from the start", pause_menu.enabled)
