...
| 2026-07-26 | 45° corners on every window: four small triangular prisms dropped into each opening's corners, so a window reads as a chamfered porthole rather than a rectangular hole. Additive, so the wall-splitting maths is untouched; tinted, because an angled face under shadowless lighting is otherwise invisible | Done, verified; three mutations killed, before/after captures | [window-chamfers.md](features/window-chamfers.md) |
