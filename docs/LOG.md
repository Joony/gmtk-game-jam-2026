...
| 2026-07-25 | Ship redrawn from a plan image (branch `ship-layout`): 4 rooms → 8 rooms + 5 corridors, transcribed pixel-for-metre from `room_layout_3.png`; engine room relocated port-aft with its 10 props; per-room light culling + per-room visual layers to fit GL Compatibility's light caps | Done, verified | [ship-layout.md](features/ship-layout.md) |
